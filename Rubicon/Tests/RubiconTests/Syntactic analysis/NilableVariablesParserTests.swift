...
final class NilableVariablesParserTests: XCTestCase {
    ...
    func test_givenClassSomeMethods_whenParse_thenReturnEmptyResult() throws {
        let code = """
        class X3Tests {
            func test_execute() {
                sut.execute(text: "")
    ...
    private func parseClass(_ code: String) throws -> ClassDeclSyntax {
        let file = SwiftParser.Parser.parse(source: code)
        if let result = file.statements.children(viewMode: .sourceAccurate).first?.as(CodeBlockItemSyntax.self)?.item.as(ClassDeclSyntax.self) {
            return result
        } else {
