...
class CreateSpyInteractorTests: XCTestCase {
    ...
    func test_givenProtocolWithExistencial_whenGenerate_thenGenerateSpy() {
        let argumentType = TypeDeclaration.makeStub(name: "Int", isOptional: true, existencial: "some")
        let argument = ArgumentDeclaration(label: "_", name: "value", type: argumentType)
        let returnType = TypeDeclaration.makeStub(name: "String", isOptional: true)
        let function = FunctionDeclaration(name: "formattedString", arguments: [argument], returnType: returnType)
        let protocolType = ProtocolDeclaration(name: "Formatter", parents: [], variables: [], functions: [function])
        ...
        equal(protocolType: protocolType, rows: [
            ...
            "\tvar formattedStringReturn: String?",
            "",
            "\tfunc formattedString(_ value: some Int?) -> String? {",
            "\t\tlet item = FormattedString(value: value)",
            "\t\tformattedString.append(item)",
            "\t\treturn formattedStringReturn",
            "\t}",
    ...
    func test_givenProtocolWithArgumentAndThrowingFunctionWithReturnValue_whenGeneratePublic_thenGenerateSpy() {
        ...
        let protocolType = ProtocolDeclaration(name: "Formatter", parents: [], variables: [variable], functions: [function])
    ...
    func test_givenProtocolSimpleFunction_whenGeneratePublic_thenGenerateSpyWithEmptyInit() {
        let function = FunctionDeclaration(name: "go", arguments: [], isThrowing: false, returnType: nil)
        let protocolType = ProtocolDeclaration(name: "Formatter", parents: [], variables: [], functions: [function])
